...
__global__ void gather(inttype *d_q, inttype *d_h, inttype *d_bits_state,
	...
	inttype* src_state = &shared[OPENTILEOFFSET+(d_sv_nints*((WARP_ID*GROUPS_PER_WARP+(LANE / d_nr_procs))))];
		...
		while (CONTINUE == 1) {
			...
			while (1) {
				...
				if (__any(i == 0)) {
					...
					for (l = 1; __any(i == 0 && l <= NR_OF_STATES_IN_TRANSENTRY(GROUP_ID)); l++) {
						if(i == 0) {
						...
						int c;
						while(generate && (c = __ballot(i == 0 && (k >> 2 == 0)))) {
